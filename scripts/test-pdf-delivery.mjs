...
const secondBytes = Buffer.from('%PDF-1.7\n% second canonical fixture\n%%EOF\n');
const secondPdfPath = path.join(packagePath, 'second-fixture.pdf');
fs.writeFileSync(path.join(recordsRoot, secondPdfPath), secondBytes);
...
fs.writeFileSync(manifestPath, JSON.stringify({ papers: [{
  ...
  paper_id: 'second-fixture', source_record_path: packagePath, pdf_path: secondPdfPath,
  annotation_path: path.join(packagePath, 'annotations-second'), document_sha256: secondHash,
  pdf_size_bytes: secondBytes.length, page_count: 1, status: 'published',
}] }));
...
try {
  ...
  const second = await fetch(`http://127.0.0.1:${port}/papers/second-fixture.pdf`);
  assert(second.status === 200 && Buffer.from(await second.arrayBuffer()).equals(secondBytes), 'second complete canonical PDF was not delivered');
  ...
  console.log('PDF delivery tests passed: every complete canonical fixture is readable and HEAD/full/Range are exact.');
} finally {
  await stop();
