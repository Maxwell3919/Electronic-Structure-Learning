#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part VII Appendix G."""
...
APPENDIX_PATH = PART_PATH + "appendix-g-stress-from-electronic-structure/"
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(
                MANIFEST_URL,
                headers={"User-Agent": "Electronic-Structure-Learning-Part07-AppG-Smoke/1"},
            )
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, PART_URL, "Appendix G")
        ...
        driver.save_screenshot(str(ARTIFACT_DIR / "part-07-index-app-g-desktop.png"))

        load_with_retry(driver, APPENDIX_URL, "Martin internal stress")
        WebDriverWait(driver, 20).until(
            lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0
        )
        title = driver.title
        if "Appendix G" not in title:
            fail(f"Unexpected Appendix G page title: {title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Appendix G desktop bilingual layout is not two columns")
        ...
        if source_rows != 5:
            fail(f"Appendix G source map should contain five rows, received {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".appendix-contents a")
        if len(contents_links) != 8:
            fail(f"Appendix G contents should contain eight links, received {len(contents_links)}")
        for link in contents_links:
            href = link.get_attribute("href") or ""
            if "/Electronic-Structure-Learning/" not in href:
                fail(f"Appendix G anchor escaped the Pages base path: {href}")
        ...
        if len(contracts) != 4 or not all(item.is_displayed() for item in contracts):
            fail(f"Appendix G should expose four visualization contracts, received {len(contracts)}")
        svgs = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual svg")
        if len(svgs) != 4 or not all(item.is_displayed() for item in svgs):
            fail(f"Appendix G should expose four static SVGs, received {len(svgs)}")

        strain_exx = driver.find_element(By.CSS_SELECTOR, "[data-strain-exx]")
        strain_polygon = driver.find_element(By.CSS_SELECTOR, "[data-strain-polygon]")
        strain_before = strain_polygon.get_attribute("points")
        focus_for_keyboard(driver, strain_exx)
        strain_exx.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: strain_polygon.get_attribute("points") != strain_before)

        elastic_amplitude = driver.find_element(By.CSS_SELECTOR, "[data-elastic-amplitude]")
        elastic_stress = driver.find_element(By.CSS_SELECTOR, "[data-elastic-sxx]")
        elastic_before = elastic_stress.text
        focus_for_keyboard(driver, elastic_amplitude)
        elastic_amplitude.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: elastic_stress.text != elastic_before)

        stress_angle = driver.find_element(By.CSS_SELECTOR, "[data-stress-angle]")
        stress_vector = driver.find_element(By.CSS_SELECTOR, "[data-stress-vector]")
        vector_before = stress_vector.get_attribute("x2")
        focus_for_keyboard(driver, stress_angle)
        stress_angle.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: stress_vector.get_attribute("x2") != vector_before)

        chain_k1 = driver.find_element(By.CSS_SELECTOR, "[data-chain-k1]")
        chain_gamma = driver.find_element(By.CSS_SELECTOR, "[data-chain-gamma]")
        gamma_before = chain_gamma.text
        focus_for_keyboard(driver, chain_k1)
        chain_k1.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: chain_gamma.text != gamma_before)
        ...
        for slug in (
            ...
            if not matches:
                fail(f"Appendix G page does not expose navigation containing {slug}")
        ...
        assert_no_page_overflow(driver, "Appendix G desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-g-desktop.png"))
        report["desktop"] = {
            "title": title,
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Appendix G narrow-screen bilingual layout is not a single column")
        assert_no_page_overflow(driver, "Appendix G narrow screen")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-g-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "appendix-g-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        ...
        contracts = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual__contract")
        if len(contracts) != 4 or not all(item.is_displayed() for item in contracts):
            fail("No-JavaScript Appendix G page lost visualization contracts")
        svgs = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual svg")
        if len(svgs) != 4 or not all(item.is_displayed() for item in svgs):
            fail("No-JavaScript Appendix G page lost static SVGs")
        fallback_selectors = (
            ".strain-metric__fallback tbody tr",
            ".elastic-stress__fallback tbody tr",
            ".stress-contribution__fallback tbody tr",
            ".internal-strain-chain__fallback tbody tr",
        )
        fallback_counts = {}
        for selector in fallback_selectors:
            ...
            fallback_counts[selector] = count
        if len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr")) != 5:
            fail("No-JavaScript Appendix G page lost source-map rows")
        if len(driver.find_elements(By.CSS_SELECTOR, ".katex")) < 90:
            fail("No-JavaScript Appendix G page lost rendered mathematics")
        assert_no_page_overflow(driver, "Appendix G no-JavaScript desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-g-no-javascript.png"))
        ...
        save_failure_screenshot(driver, "appendix-g-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
def main() -> int:
    ...
    report_path = ARTIFACT_DIR / "part07-app-g-report.json"
...
if __name__ == "__main__":
    try:
        ...
        (ARTIFACT_DIR / "part07-app-g-failure.json").write_text(
            json.dumps(failure, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        print(f"Part VII Appendix G Pages smoke test failed: {exc}", file=sys.stderr)
        raise
