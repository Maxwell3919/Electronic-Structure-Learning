#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part II Chapter 7."""
...
CHAPTER_PATH = PART_PATH + "chapter-07-the-kohn-sham-auxiliary-system/"
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(
                MANIFEST_URL,
                headers={"User-Agent": "Electronic-Structure-Learning-Part02-Ch07-Smoke/1"},
            )
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        ...
        driver.save_screenshot(str(ARTIFACT_DIR / "part-02-index-ch07-desktop.png"))

        load_with_retry(driver, CHAPTER_URL, "Kohn–Sham Auxiliary System")
        WebDriverWait(driver, 20).until(
            lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0
        )

        title = driver.title
        if "Chapter 7" not in title:
            fail(f"Unexpected Chapter 7 page title: {title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Chapter 7 desktop bilingual layout is not two columns")
        ...
        if source_rows != 10:
            fail(f"Chapter 7 source map should contain ten sections, received {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".chapter-contents a")
        if len(contents_links) < 14:
            fail("Chapter 7 explicit contents is incomplete")
        ...
        # Energy double-counting model.
        hartree_slider = driver.find_element(By.CSS_SELECTOR, "[data-kse-eh]")
        total_energy = driver.find_element(By.CSS_SELECTOR, "[data-kse-total]")
        total_before = total_energy.text
        focus_for_keyboard(driver, hartree_slider)
        hartree_slider.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: total_energy.text != total_before)

        # Occupation-density model.
        electron_slider = driver.find_element(By.CSS_SELECTOR, "[data-od-electrons]")
        integral_output = driver.find_element(By.CSS_SELECTOR, "[data-od-integral]")
        focus_for_keyboard(driver, electron_slider)
        electron_slider.send_keys(Keys.HOME)
        WebDriverWait(driver, 10).until(lambda _: "1.000000" in integral_output.text)
        electron_slider.send_keys(Keys.END)
        WebDriverWait(driver, 10).until(lambda _: "8.000000" in integral_output.text)

        # Shared SCF model and preset buttons.
        slope_slider = driver.find_element(By.CSS_SELECTOR, "[data-scf-visualizer] [data-slope]")
        q_output = driver.find_element(By.CSS_SELECTOR, "[data-scf-visualizer] [data-q]")
        q_before = q_output.text
        focus_for_keyboard(driver, slope_slider)
        slope_slider.send_keys(Keys.ARROW_LEFT)
        WebDriverWait(driver, 10).until(lambda _: q_output.text != q_before)

        divergent_button = driver.find_element(By.CSS_SELECTOR, "[data-scf-visualizer] [data-preset='divergent']")
        focus_for_keyboard(driver, divergent_button)
        divergent_button.send_keys(Keys.ENTER)
        mode_output = driver.find_element(By.CSS_SELECTOR, "[data-scf-visualizer] [data-mode]")
        WebDriverWait(driver, 10).until(lambda _: "发散" in mode_output.text)
        ...
        if not all(contract.is_displayed() for contract in contracts):
            fail("A Chapter 7 visualization contract is hidden")

        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-07-desktop.png"))
        report["desktop"] = {
            "title": title,
            ...
            "keyboard_controls": ["hartree_energy", "electron_count", "scf_slope", "scf_divergent_preset"],
        }
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Chapter 7 narrow-screen bilingual layout is not a single column")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-07-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "chapter-07-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, CHAPTER_URL, "Original static diagram")
        contracts = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual__contract")
        if len(contracts) != 4 or not all(item.is_displayed() for item in contracts):
            fail("No-JavaScript page does not retain all four visible visualization contracts")
        ...
        if len(static_svgs) != 4 or not all(item.is_displayed() for item in static_svgs):
            fail(f"No-JavaScript page should retain four direct static SVG fallbacks, received {len(static_svgs)}")
        ...
        fallback_row = driver.find_element(By.CSS_SELECTOR, "[data-scf-visualizer] [data-rows]")
        if "JavaScript" not in fallback_row.text:
            fail("Shared SCF component lost its no-JavaScript table fallback")

        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-07-no-javascript.png"))
        report["no_javascript"] = {
            "visualization_contracts": len(contracts),
            "direct_static_svg_count": len(static_svgs),
            "source_map_rows": 10,
            "scf_table_fallback": True,
        }
    except Exception:
        save_failure_screenshot(driver, "chapter-07-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
def main() -> int:
    ...
    report_path = ARTIFACT_DIR / "part02-ch07-report.json"
...
if __name__ == "__main__":
    try:
        ...
        (ARTIFACT_DIR / "part02-ch07-failure.json").write_text(
            json.dumps(failure, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        print(f"Part II Chapter 7 Pages smoke test failed: {exc}", file=sys.stderr)
        raise
