#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part II Chapter 6."""
...
CHAPTER_PATH = PART_PATH + "chapter-06-density-functional-theory-foundations/"
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(
                MANIFEST_URL,
                headers={"User-Agent": "Electronic-Structure-Learning-Part02-Smoke/1"},
            )
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, PART_URL, "Logical sequence of the four chapters")
        ...
        driver.save_screenshot(str(ARTIFACT_DIR / "part-02-index-desktop.png"))

        load_with_retry(driver, CHAPTER_URL, "Hohenberg–Kohn")
        WebDriverWait(driver, 20).until(
            lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0
        )

        title = driver.title
        if "Chapter 6" not in title:
            fail(f"Unexpected Chapter 6 page title: {title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Chapter 6 desktop bilingual layout is not two columns")

        katex_count = len(driver.find_elements(By.CSS_SELECTOR, ".katex"))
        if katex_count < 20:
            fail(f"Expected at least 20 rendered KaTeX expressions, received {katex_count}")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows != 7:
            fail(f"Chapter 6 source map should contain seven sections, received {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".chapter-contents a")
        if len(contents_links) < 11:
            fail("Chapter 6 explicit contents is incomplete")
        ...
        # Potential map: changing a common shift changes energy but not density.
        shift_slider = driver.find_element(By.CSS_SELECTOR, "[data-pdm-shift]")
        density_output = driver.find_element(By.CSS_SELECTOR, "[data-pdm-density]")
        energy_output = driver.find_element(By.CSS_SELECTOR, "[data-pdm-energy]")
        density_before = density_output.text
        energy_before = energy_output.text
        focus_for_keyboard(driver, shift_slider)
        shift_slider.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: energy_output.text != energy_before)
        if density_output.text != density_before:
            fail("Common potential shift changed the two-site density")

        delta_slider = driver.find_element(By.CSS_SELECTOR, "[data-pdm-delta]")
        focus_for_keyboard(driver, delta_slider)
        delta_slider.send_keys(Keys.HOME)
        density_at_lower_bound = density_output.text
        delta_slider.send_keys(Keys.END)
        WebDriverWait(driver, 10).until(lambda _: density_output.text != density_at_lower_bound)

        # Constrained search: keyboard controls must update inner and outer searches.
        trial_slider = driver.find_element(By.CSS_SELECTOR, "[data-cs-trial]")
        inner_output = driver.find_element(By.CSS_SELECTOR, "[data-cs-inner]")
        focus_for_keyboard(driver, trial_slider)
        trial_slider.send_keys(Keys.HOME)
        inner_at_lower_bound = inner_output.text
        trial_slider.send_keys(Keys.END)
        WebDriverWait(driver, 10).until(lambda _: inner_output.text != inner_at_lower_bound)

        slope_slider = driver.find_element(By.CSS_SELECTOR, "[data-cs-slope]")
        outer_output = driver.find_element(By.CSS_SELECTOR, "[data-cs-outer]")
        focus_for_keyboard(driver, slope_slider)
        slope_slider.send_keys(Keys.HOME)
        outer_at_lower_bound = outer_output.text
        slope_slider.send_keys(Keys.END)
        WebDriverWait(driver, 10).until(lambda _: outer_output.text != outer_at_lower_bound)

        contracts = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual__contract")
        if len(contracts) != 3:
            fail(f"Expected three visualization contracts, received {len(contracts)}")
        if not all(contract.is_displayed() for contract in contracts):
            fail("A Chapter 6 visualization contract is hidden")

        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-06-desktop.png"))
        report["desktop"] = {
            "title": title,
            ...
            "keyboard_controls": ["potential_shift", "potential_difference", "trial_density", "external_slope"],
        }
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Chapter 6 narrow-screen bilingual layout is not a single column")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-06-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "chapter-06-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        ...
        if len(contracts) != 3 or not all(item.is_displayed() for item in contracts):
            fail("No-JavaScript page does not retain all three visible visualization contracts")
        ...
        if len(static_svgs) != 3 or not all(item.is_displayed() for item in static_svgs):
            fail("No-JavaScript page does not retain all three static SVG fallbacks")
        if len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr")) != 7:
            fail("No-JavaScript page lost the seven-row source map")

        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-06-no-javascript.png"))
        ...
        save_failure_screenshot(driver, "chapter-06-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
def main() -> int:
    ...
    report_path = ARTIFACT_DIR / "part02-ch06-report.json"
...
if __name__ == "__main__":
    try:
        ...
        (ARTIFACT_DIR / "part02-ch06-failure.json").write_text(
            json.dumps(failure, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        print(f"Part II Chapter 6 Pages smoke test failed: {exc}", file=sys.stderr)
        raise
