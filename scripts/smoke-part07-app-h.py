#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part VII Appendix H."""
...
APPENDIX_PATH = PART_PATH + "appendix-h-energy-and-stress-densities/"
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(
                MANIFEST_URL,
                headers={"User-Agent": "Electronic-Structure-Learning-Part07-AppH-Smoke/1"},
            )
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, PART_URL, "Appendix H")
        ...
        driver.save_screenshot(str(ARTIFACT_DIR / "part-07-index-app-h-desktop.png"))

        load_with_retry(driver, APPENDIX_URL, "normalization/notation mismatch")
        WebDriverWait(driver, 20).until(
            lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0
        )
        title = driver.title
        if "Appendix H" not in title:
            fail(f"Unexpected Appendix H page title: {title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Appendix H desktop bilingual layout is not two columns")

        katex_count = len(driver.find_elements(By.CSS_SELECTOR, ".katex"))
        if katex_count < 110:
            fail(f"Expected at least 110 rendered KaTeX expressions, received {katex_count}")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows != 5:
            fail(f"Appendix H source map should contain five rows, received {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".appendix-contents a")
        if len(contents_links) != 8:
            fail(f"Appendix H contents should contain eight links, received {len(contents_links)}")
        for link in contents_links:
            href = link.get_attribute("href") or ""
            if "/Electronic-Structure-Learning/" not in href:
                fail(f"Appendix H anchor escaped the Pages base path: {href}")
        ...
        if len(contracts) != 4 or not all(item.is_displayed() for item in contracts):
            fail(f"Appendix H should expose four visualization contracts, received {len(contracts)}")
        svgs = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual svg")
        if len(svgs) != 4 or not all(item.is_displayed() for item in svgs):
            fail(f"Appendix H should expose four static SVGs, received {len(svgs)}")

        energy_amplitude = driver.find_element(By.CSS_SELECTOR, "[data-energy-gauge-amplitude]")
        energy_path = driver.find_element(By.CSS_SELECTOR, "[data-energy-gauge-path]")
        energy_before = energy_path.get_attribute("d")
        focus_for_keyboard(driver, energy_amplitude)
        energy_amplitude.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: energy_path.get_attribute("d") != energy_before)

        kinetic_amplitude = driver.find_element(By.CSS_SELECTOR, "[data-kinetic-amplitude]")
        kinetic_path = driver.find_element(By.CSS_SELECTOR, "[data-kinetic-t2-path]")
        kinetic_before = kinetic_path.get_attribute("d")
        focus_for_keyboard(driver, kinetic_amplitude)
        kinetic_amplitude.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: kinetic_path.get_attribute("d") != kinetic_before)

        stress_amplitude = driver.find_element(By.CSS_SELECTOR, "[data-stress-gauge-amplitude]")
        stress_path = driver.find_element(By.CSS_SELECTOR, "[data-stress-gauge-path]")
        stress_before = stress_path.get_attribute("d")
        focus_for_keyboard(driver, stress_amplitude)
        stress_amplitude.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: stress_path.get_attribute("d") != stress_before)

        elf_baseline = driver.find_element(By.CSS_SELECTOR, "[data-elf-baseline]")
        elf_path = driver.find_element(By.CSS_SELECTOR, "[data-elf-path]")
        elf_before = elf_path.get_attribute("d")
        focus_for_keyboard(driver, elf_baseline)
        elf_baseline.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: elf_path.get_attribute("d") != elf_before)
        ...
        for slug in (
            ...
            if not matches:
                fail(f"Appendix H page does not expose navigation containing {slug}")
        ...
        assert_no_page_overflow(driver, "Appendix H desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-h-desktop.png"))
        report["desktop"] = {
            "title": title,
            ...
            "keyboard_controls": [
                "energy_gauge_amplitude",
                "kinetic_amplitude",
                "stress_gauge_amplitude",
                "elf_baseline",
            ],
        }
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Appendix H narrow-screen bilingual layout is not a single column")
        assert_no_page_overflow(driver, "Appendix H narrow screen")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-h-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "appendix-h-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        ...
        contracts = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual__contract")
        if len(contracts) != 4 or not all(item.is_displayed() for item in contracts):
            fail("No-JavaScript Appendix H page lost visualization contracts")
        svgs = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual svg")
        if len(svgs) != 4 or not all(item.is_displayed() for item in svgs):
            fail("No-JavaScript Appendix H page lost static SVGs")
        fallback_selectors = (
            ".energy-gauge__fallback tbody tr",
            ".kinetic-gauge__fallback tbody tr",
            ".stress-gauge-flux__fallback tbody tr",
            ".elf-explorer__fallback tbody tr",
        )
        for selector in fallback_selectors:
            count = len(driver.find_elements(By.CSS_SELECTOR, selector))
            if count != 3:
                fail(f"No-JavaScript fallback {selector} should contain three rows, received {count}")
        if len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr")) != 5:
            fail("No-JavaScript Appendix H page lost source-map rows")
        if len(driver.find_elements(By.CSS_SELECTOR, ".katex")) < 110:
            fail("No-JavaScript Appendix H page lost rendered mathematics")
        assert_no_page_overflow(driver, "Appendix H no-JavaScript desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-h-no-javascript.png"))
        report["no_javascript"] = {
            "visualization_contracts": len(contracts),
            "static_svg_count": len(svgs),
            "source_map_rows": 5,
            "fallback_tables": len(fallback_selectors),
        }
    except Exception:
        save_failure_screenshot(driver, "appendix-h-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
def main() -> int:
    ...
    report_path = ARTIFACT_DIR / "part07-app-h-report.json"
...
if __name__ == "__main__":
    try:
        ...
        (ARTIFACT_DIR / "part07-app-h-failure.json").write_text(
            json.dumps(failure, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        print(f"Part VII Appendix H Pages smoke test failed: {exc}", file=sys.stderr)
        raise
