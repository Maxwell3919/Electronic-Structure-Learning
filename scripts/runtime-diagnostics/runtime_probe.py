...
(() => {
  ...
  const state = {
    ...
    timeoutsCreated: 0, timeoutsCleared: 0, timeouts: new Set(),
    intervalsCreated: 0, intervalsCleared: 0, intervals: new Set(),
    rafCreated: 0, rafCancelled: 0, raf: new Set(),
  ...
  EventTarget.prototype.addEventListener = function(type, listener, options) {
    if (listener) {
      ...
      state.listeners.push({ target: this, type, listener, capture: capture(options) });
    }
  ...
  globalThis.setTimeout = (callback, delay, ...args) => {
    ...
    id = nativeTimeout(wrapped, delay, ...args); state.timeoutsCreated += 1; state.timeouts.add(id); return id;
  };
  ...
  globalThis.setInterval = (callback, delay, ...args) => { const id = nativeInterval(callback, delay, ...args); state.intervalsCreated += 1; state.intervals.add(id); return id; };
  ...
  globalThis.requestAnimationFrame = (callback) => { let id; id = nativeRaf((time) => { state.raf.delete(id); callback(time); }); state.rafCreated += 1; state.raf.add(id); return id; };
