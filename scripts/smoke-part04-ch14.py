#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part IV Chapter 14."""
...
CHAPTER_PATH = PART_PATH + "chapter-14-localized-orbitals-tight-binding/"
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(
                MANIFEST_URL,
                headers={"User-Agent": "Electronic-Structure-Learning-Part04-Ch14-Smoke/1"},
            )
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        ...
        if "Chapter 13" not in driver.page_source or "Chapter 14" not in driver.page_source:
            fail("Part IV index does not expose Chapters 13 and 14")
        assert_no_page_overflow(driver, "Part IV desktop index")
        driver.save_screenshot(str(ARTIFACT_DIR / "part-04-index-ch14-desktop.png"))

        load_with_retry(driver, CHAPTER_URL, "Transferability: Nonorthogonality and Environment Dependence")
        WebDriverWait(driver, 20).until(
            lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0
        )
        title = driver.title
        if "Chapter 14" not in title:
            fail(f"Unexpected Chapter 14 page title: {title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Chapter 14 desktop bilingual layout is not two columns")

        katex_count = len(driver.find_elements(By.CSS_SELECTOR, ".katex"))
        if katex_count < 100:
            fail(f"Expected at least 100 rendered KaTeX expressions, received {katex_count}")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows != 12:
            fail(f"Chapter 14 source map should contain twelve rows, received {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".chapter14-contents a")
        if len(contents_links) != 16:
            fail(f"Chapter 14 contents should contain sixteen links, received {len(contents_links)}")
        for link in contents_links:
            href = link.get_attribute("href") or ""
            if "/Electronic-Structure-Learning/" not in href:
                fail(f"Chapter 14 anchor escaped the Pages base path: {href}")
        ...
        if len(contracts) != 5 or not all(item.is_displayed() for item in contracts):
            fail(f"Chapter 14 must expose five visible visualization contracts, found {len(contracts)}")
        if len(static_svgs) != 5 or not all(item.is_displayed() for item in static_svgs):
            fail(f"Chapter 14 must expose five visible SVGs, found {len(static_svgs)}")

        controls = [
            ("[data-chain-hopping]", "[data-chain-hopping-output]", "chain hopping"),
            ("[data-sk-angle]", "[data-sk-sp]", "Slater-Koster orientation"),
            ("[data-two-coupling]", "[data-two-gap]", "two-band coupling"),
            ("[data-graphene-mass]", "[data-graphene-gap]", "graphene sublattice mass"),
            ("[data-tube-n]", "[data-tube-index]", "nanotube chiral index"),
        ]
        for control, output, label in controls:
            update_range_and_require_change(driver, control, output, label)

        factor_text = driver.find_element(By.CSS_SELECTOR, "[data-graphene-factor]").text
        if abs(float(factor_text)) > 1e-10:
            fail(f"Graphene teaching model lost its K-point structure-factor zero: {factor_text}")
        ...
        assert_no_page_overflow(driver, "Chapter 14 desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-14-desktop.png"))
        report["desktop"] = {
            "title": title,
            ...
            "graphene_factor_at_k": factor_text,
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Chapter 14 narrow-screen bilingual layout is not one column")
        assert_no_page_overflow(driver, "Chapter 14 narrow screen")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-14-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "chapter-14-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        ...
        if len(contracts) != 5 or not all(item.is_displayed() for item in contracts):
            fail("No-JavaScript Chapter 14 page lost a visualization contract")
        if len(svgs) != 5 or not all(item.is_displayed() for item in svgs):
            fail("No-JavaScript Chapter 14 page lost a static SVG")
        if len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr")) != 12:
            fail("No-JavaScript Chapter 14 page lost a source-map row")
        fallbacks = driver.find_elements(By.XPATH, "//*[contains(text(), '关闭 JavaScript')]")
        if len(fallbacks) < 5:
            fail(f"Chapter 14 should expose five no-JavaScript fallback explanations, found {len(fallbacks)}")
        assert_no_page_overflow(driver, "Chapter 14 no-JavaScript")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-14-no-javascript.png"))
        ...
        save_failure_screenshot(driver, "chapter-14-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
def main() -> int:
    ...
    report_path = ARTIFACT_DIR / "part04-ch14-report.json"
...
if __name__ == "__main__":
    try:
        ...
        (ARTIFACT_DIR / "part04-ch14-failure.json").write_text(
            json.dumps(failure, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        print(f"Part IV Chapter 14 Pages smoke test failed: {exc}", file=sys.stderr)
        raise
