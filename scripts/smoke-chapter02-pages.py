...
CHAPTER_URL = urljoin(BASE_URL, "part-01-overview-and-background/chapter-02-overview/")
...
def load_with_retry(driver: webdriver.Chrome, marker: str, attempts: int = 12) -> None:
    ...
    for _ in range(attempts):
        try:
            ...
            return
        except Exception as exc:  # Selenium uses several transient exception types.
            last_error = exc
            time.sleep(5)
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, "Chapter 2")
        ...
        WebDriverWait(driver, 10).until(
            lambda _: pressure_slider.get_attribute("value") == "0"
            and "Stable phase: A" in stable_phase.text
        )
        pressure_slider.send_keys(Keys.END)
        WebDriverWait(driver, 10).until(
            lambda _: pressure_slider.get_attribute("value") == "0.3"
            and "Stable phase: B" in stable_phase.text
        )
        ...
        WebDriverWait(driver, 10).until(
            ...
            and "property-router__card--active"
            in (topology_card.get_attribute("class") or "")
        )
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, "Original teaching model")
...
def main() -> int:
    ...
    (ARTIFACT_DIR / "chapter-02-report.json").write_text(
