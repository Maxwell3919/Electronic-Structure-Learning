#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part V Chapter 21."""
...
ARTIFACT_DIR = Path(os.environ.get("SMOKE_ARTIFACT_DIR", "artifacts/pages-smoke")) / "chapter-21"
...
CHAPTER_URL = urljoin(
    BASE_URL,
    "part-05-properties-of-matter/chapter-21-excitation-spectra-and-optical-properties/",
)
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(
                url,
                headers={"User-Agent": "Electronic-Structure-Learning-Ch21-Smoke/1"},
            )
...
def part_index_smoke(report: dict) -> None:
    ...
    try:
        ...
        links = driver.find_elements(
            By.CSS_SELECTOR,
            'a[href*="chapter-21-excitation-spectra-and-optical-properties"]',
        )
        if not links:
            fail("Part V index lacks a Chapter 21 link")
        href = links[0].get_attribute("href") or ""
        if "/Electronic-Structure-Learning/" not in href:
            fail(f"Chapter 21 link escaped the Pages base path: {href}")
        assert_no_page_overflow(driver, "Part V index")
        driver.save_screenshot(str(ARTIFACT_DIR / "part-05-index.png"))
        report["part_index"] = {"dependency_stages": stages, "chapter_21_link": href}
    except Exception:
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, CHAPTER_URL, "Chapter 21")
        WebDriverWait(driver, 20).until(
            lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0
        )
        if "Chapter 21" not in driver.title:
            fail(f"Unexpected Chapter 21 title: {driver.title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Chapter 21 desktop bilingual layout is not two columns")

        katex_count = len(driver.find_elements(By.CSS_SELECTOR, ".katex"))
        if katex_count < 60:
            fail(f"Expected at least 60 rendered KaTeX nodes, found {katex_count}")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows != 9:
            fail(f"Chapter 21 source map must contain nine sections, found {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".chapter21-contents a")
        if len(contents_links) != 13:
            fail(f"Chapter 21 contents must contain thirteen links, found {len(contents_links)}")

        move_slider_and_expect_change(
            driver,
            "#ch21-two-level-polarizability [data-transition]",
            "#ch21-two-level-polarizability [data-transition-output]",
        )
        move_slider_and_expect_change(
            driver,
            "#ch21-coupled-transitions [data-coupling]",
            "#ch21-coupled-transitions [data-lower-energy]",
        )
        move_slider_and_expect_change(
            driver,
            "#ch21-real-time-resolution [data-total-time]",
            "#ch21-real-time-resolution [data-resolution-output]",
        )
        move_slider_and_expect_change(
            driver,
            "#ch21-exciton-binding [data-attraction]",
            "#ch21-exciton-binding [data-lower-energy]",
        )
        move_slider_and_expect_change(
            driver,
            "#ch21-dielectric-local-field [data-wing]",
            "#ch21-dielectric-local-field [data-macro-output]",
        )

        assert_no_page_overflow(driver, "Chapter 21 desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-21-desktop.png"))
        report["desktop"] = {
            "title": driver.title,
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Chapter 21 narrow bilingual layout is not a single column")
        assert_no_page_overflow(driver, "Chapter 21 narrow")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-21-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "chapter-21-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        ...
        if len(contracts) != 5:
            fail(f"Chapter 21 must retain five visualization contracts, found {len(contracts)}")
        if len(figures) != 5:
            fail(f"Chapter 21 must retain five teaching figures, found {len(figures)}")
        if len(fallbacks) < 5:
            fail(f"Chapter 21 must expose five no-JavaScript explanations, found {len(fallbacks)}")
        if not all(item.is_displayed() for item in contracts + figures):
            fail("A Chapter 21 no-JavaScript figure or contract is hidden")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows != 9:
            fail("No-JavaScript Chapter 21 source map lost one or more sections")
        assert_no_page_overflow(driver, "Chapter 21 no-JavaScript")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-21-no-javascript.png"))
        ...
        save_failure_screenshot(driver, "chapter-21-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
if __name__ == "__main__":
    try:
        ...
        print(f"Chapter 21 Pages smoke test failed: {exc}", file=sys.stderr)
        raise
