#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part VII Appendix F."""
...
APPENDIX_PATH = PART_PATH + "appendix-f-coulomb-interactions-in-extended-systems/"
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(
                MANIFEST_URL,
                headers={"User-Agent": "Electronic-Structure-Learning-Part07-AppF-Smoke/1"},
            )
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, PART_URL, "Appendix F")
        if "Part VII" not in driver.title:
            fail(f"Unexpected Part VII page title: {driver.title}")
        for label in ("Appendix A", "Appendix B", "Appendix C", "Appendix D", "Appendix E", "Appendix F"):
        ...
        driver.save_screenshot(str(ARTIFACT_DIR / "part-07-index-app-f-desktop.png"))

        load_with_retry(driver, APPENDIX_URL, "Ewald")
        WebDriverWait(driver, 20).until(
            lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0
        )
        title = driver.title
        if "Appendix F" not in title:
            fail(f"Unexpected Appendix F page title: {title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Appendix F desktop bilingual layout is not two columns")

        katex_count = len(driver.find_elements(By.CSS_SELECTOR, ".katex"))
        if katex_count < 60:
            fail(f"Expected at least 60 rendered KaTeX expressions, received {katex_count}")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows != 7:
            fail(f"Appendix F source map should contain seven rows, received {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".appendix-contents a")
        if len(contents_links) != 10:
            fail(f"Appendix F contents should contain ten links, received {len(contents_links)}")
        for link in contents_links:
            href = link.get_attribute("href") or ""
            if "/Electronic-Structure-Learning/" not in href:
                fail(f"Appendix F anchor escaped the Pages base path: {href}")
        ...
        if len(contracts) != 4 or not all(item.is_displayed() for item in contracts):
            fail(f"Appendix F should expose four visualization contracts, received {len(contracts)}")
        svgs = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual svg")
        if len(svgs) != 4 or not all(item.is_displayed() for item in svgs):
            fail(f"Appendix F should expose four static SVGs, received {len(svgs)}")

        ewald_alpha = driver.find_element(By.CSS_SELECTOR, "[data-ewald-alpha]")
        ewald_probe = driver.find_element(By.CSS_SELECTOR, "[data-ewald-probe]")
        ewald_probe_before = ewald_probe.get_attribute("x1")
        focus_for_keyboard(driver, ewald_alpha)
        ewald_alpha.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: ewald_probe.get_attribute("x1") != ewald_probe_before)

        gaussian_width = driver.find_element(By.CSS_SELECTOR, "[data-gaussian-width]")
        gaussian_path = driver.find_element(By.CSS_SELECTOR, "[data-gaussian-smeared-path]")
        gaussian_path_before = gaussian_path.get_attribute("d")
        focus_for_keyboard(driver, gaussian_width)
        gaussian_width.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: gaussian_path.get_attribute("d") != gaussian_path_before)

        slab_cell = driver.find_element(By.CSS_SELECTOR, "[data-slab-cell]")
        slab_image = driver.find_element(By.CSS_SELECTOR, "[data-slab-image]")
        slab_image_before = slab_image.text
        focus_for_keyboard(driver, slab_cell)
        slab_cell.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: slab_image.text != slab_image_before)
        ...
        for slug in (
            ...
            if not matches:
                fail(f"Appendix F page does not expose navigation containing {slug}")
        ...
        assert_no_page_overflow(driver, "Appendix F desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-f-desktop.png"))
        report["desktop"] = {
            "title": title,
            ...
            "keyboard_controls": ["ewald_alpha", "gaussian_width", "slab_cell_length"],
        }
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Appendix F narrow-screen bilingual layout is not a single column")
        assert_no_page_overflow(driver, "Appendix F narrow screen")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-f-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "appendix-f-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        ...
        contracts = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual__contract")
        if len(contracts) != 4 or not all(item.is_displayed() for item in contracts):
            fail("No-JavaScript Appendix F page lost visualization contracts")
        svgs = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual svg")
        if len(svgs) != 4 or not all(item.is_displayed() for item in svgs):
            fail("No-JavaScript Appendix F page lost static SVGs")
        if len(driver.find_elements(By.CSS_SELECTOR, ".ewald-explorer__fallback tbody tr")) != 3:
            fail("No-JavaScript Ewald table does not contain three benchmark rows")
        if len(driver.find_elements(By.CSS_SELECTOR, ".gaussian-charge__fallback tbody tr")) != 3:
            fail("No-JavaScript Gaussian table does not contain three benchmark rows")
        if len(driver.find_elements(By.CSS_SELECTOR, ".slab-dipole__fallback tbody tr")) != 3:
            fail("No-JavaScript slab-dipole table does not contain three benchmark rows")
        if len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr")) != 7:
            fail("No-JavaScript Appendix F page lost source-map rows")
        if len(driver.find_elements(By.CSS_SELECTOR, ".katex")) < 60:
            fail("No-JavaScript Appendix F page lost rendered mathematics")
        assert_no_page_overflow(driver, "Appendix F no-JavaScript desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-f-no-javascript.png"))
        ...
        save_failure_screenshot(driver, "appendix-f-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
def main() -> int:
    ...
    report_path = ARTIFACT_DIR / "part07-app-f-report.json"
...
if __name__ == "__main__":
    try:
        ...
        (ARTIFACT_DIR / "part07-app-f-failure.json").write_text(
            json.dumps(failure, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        print(f"Part VII Appendix F Pages smoke test failed: {exc}", file=sys.stderr)
        raise
