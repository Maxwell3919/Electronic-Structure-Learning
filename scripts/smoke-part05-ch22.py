#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part V Chapter 22."""
...
ARTIFACT_DIR = Path(os.environ.get("SMOKE_ARTIFACT_DIR", "artifacts/pages-smoke")) / "chapter-22"
ARTIFACT_DIR.mkdir(parents=True, exist_ok=True)
BASE_URL = os.environ["PAGES_URL"].rstrip("/") + "/"
EXPECTED_SHA = os.environ["DEPLOYED_SHA"]
PART_URL = urljoin(BASE_URL, "part-05-properties-of-matter/")
CHAPTER_URL = urljoin(
    BASE_URL,
    "part-05-properties-of-matter/chapter-22-surfaces-interfaces-and-lower-dimensional-systems/",
)
...
def fetch_json_with_retry(url: str, attempts: int = 30, delay: float = 5.0) -> dict:
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "Electronic-Structure-Learning-Ch22-Smoke/1"})
...
def focus_for_keyboard(driver: webdriver.Chrome, element: WebElement) -> None:
    driver.execute_script(
        "arguments[0].scrollIntoView({block:'center',inline:'nearest'}); arguments[0].focus({preventScroll:true});",
        element,
    )
...
def move_control_and_expect_change(
...
def part_index_smoke(report: dict) -> None:
    ...
    try:
        ...
        stages = len(driver.find_elements(By.CSS_SELECTOR, ".part05-chain span"))
        if stages != 5:
            fail(f"Part V dependency chain must contain five stages, found {stages}")
        links = driver.find_elements(By.CSS_SELECTOR, 'a[href*="chapter-22-surfaces-interfaces-and-lower-dimensional-systems"]')
        if not links:
            fail("Part V index lacks a Chapter 22 link")
        href = links[0].get_attribute("href") or ""
        if "/Electronic-Structure-Learning/" not in href:
            fail(f"Chapter 22 link escaped the Pages base path: {href}")
        assert_no_page_overflow(driver, "Part V index")
        driver.save_screenshot(str(ARTIFACT_DIR / "part-05-index.png"))
        report["part_index"] = {"dependency_stages": stages, "chapter_22_link": href}
    except Exception:
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, CHAPTER_URL, "Chapter 22")
        WebDriverWait(driver, 20).until(lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0)
        if "Chapter 22" not in driver.title:
            fail(f"Unexpected Chapter 22 title: {driver.title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Chapter 22 desktop bilingual layout is not two columns")

        katex_count = len(driver.find_elements(By.CSS_SELECTOR, ".katex"))
        if katex_count < 100:
            fail(f"Expected at least 100 rendered KaTeX nodes, found {katex_count}")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows < 9:
            fail(f"Chapter 22 source map must contain at least nine rows, found {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".chapter-contents a")
        if len(contents_links) != 14:
            fail(f"Chapter 22 contents must contain fourteen links, found {len(contents_links)}")
        figures = driver.find_elements(By.CSS_SELECTOR, ".chapter22-root .chapter-visual")
        contracts = driver.find_elements(By.CSS_SELECTOR, ".chapter22-root .chapter-visual__contract")
        if len(figures) != 5 or len(contracts) != 5:
            fail(f"Chapter 22 requires five figures/contracts, found {len(figures)}/{len(contracts)}")

        move_control_and_expect_change(
            driver,
            "#ch22-slab-work-function [data-left]",
            "#ch22-slab-work-function [data-left-vac]",
        )
        move_control_and_expect_change(
            driver,
            "#ch22-surface-state [data-boundary]",
            "#ch22-surface-state [data-energy]",
        )
        move_control_and_expect_change(
            driver,
            "#ch22-interface-lineup [data-lineup]",
            "#ch22-interface-lineup [data-vbo]",
        )
        move_control_and_expect_change(
            driver,
            "#ch22-polar-discontinuity [data-compensation]",
            "#ch22-polar-discontinuity [data-slope]",
        )
        move_control_and_expect_change(
            driver,
            "#ch22-dimensional-dos [data-broadening]",
            "#ch22-dimensional-dos [data-peak]",
        )

        assert_no_page_overflow(driver, "Chapter 22 desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-22-desktop.png"))
        report["desktop"] = {
            "title": driver.title,
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Chapter 22 narrow bilingual layout is not a single column")
        assert_no_page_overflow(driver, "Chapter 22 narrow")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-22-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "chapter-22-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        ...
        if len(contracts) != 5:
            fail(f"Chapter 22 must retain five visualization contracts, found {len(contracts)}")
        if len(figures) != 5:
            fail(f"Chapter 22 must retain five teaching figures, found {len(figures)}")
        if len(fallbacks) < 5:
            fail(f"Chapter 22 must expose five no-JavaScript explanations, found {len(fallbacks)}")
        if not all(item.is_displayed() for item in contracts + figures):
            fail("A Chapter 22 no-JavaScript figure or contract is hidden")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows < 9:
            fail("No-JavaScript Chapter 22 source map lost one or more sections")
        assert_no_page_overflow(driver, "Chapter 22 no-JavaScript")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-22-no-javascript.png"))
        ...
        save_failure_screenshot(driver, "chapter-22-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
if __name__ == "__main__":
    try:
        ...
        print(f"Chapter 22 Pages smoke test failed: {exc}", file=sys.stderr)
        raise
