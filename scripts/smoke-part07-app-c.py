#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part VII Appendix C."""
...
APPENDIX_PATH = PART_PATH + "appendix-c-adiabatic-approximation/"
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(
                MANIFEST_URL,
                headers={"User-Agent": "Electronic-Structure-Learning-Part07-AppC-Smoke/1"},
            )
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, PART_URL, "Appendix C")
        if "Part VII" not in driver.title:
            fail(f"Unexpected Part VII page title: {driver.title}")
        for label in ("Appendix A", "Appendix B", "Appendix C"):
        ...
        driver.save_screenshot(str(ARTIFACT_DIR / "part-07-index-app-c-desktop.png"))

        load_with_retry(driver, APPENDIX_URL, "derivative coupling")
        WebDriverWait(driver, 20).until(
            lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0
        )
        title = driver.title
        if "Appendix C" not in title:
            fail(f"Unexpected Appendix C page title: {title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Appendix C desktop bilingual layout is not two columns")
        ...
        if source_rows != 3:
            fail(f"Appendix C source map should contain three rows, received {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".appendix-contents a")
        if len(contents_links) != 10:
            fail(f"Appendix C contents should contain ten links, received {len(contents_links)}")
        for link in contents_links:
            href = link.get_attribute("href") or ""
            if "/Electronic-Structure-Learning/" not in href:
                fail(f"Appendix C anchor escaped the Pages base path: {href}")
        ...
        if len(contracts) != 3 or not all(item.is_displayed() for item in contracts):
            fail("Appendix C does not expose all three visualization contracts")
        if len(static_svgs) != 3 or not all(item.is_displayed() for item in static_svgs):
            fail("Appendix C does not expose all three static SVGs")

        coupling_slider = driver.find_element(By.CSS_SELECTOR, "[data-ac-coupling]")
        min_gap = driver.find_element(By.CSS_SELECTOR, "[data-ac-min-gap]")
        gap_before = min_gap.text
        focus_for_keyboard(driver, coupling_slider)
        coupling_slider.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: min_gap.text != gap_before)

        velocity_slider = driver.find_element(By.CSS_SELECTOR, "[data-ac-velocity]")
        indicator = driver.find_element(By.CSS_SELECTOR, "[data-ac-indicator]")
        indicator_before = indicator.text
        focus_for_keyboard(driver, velocity_slider)
        velocity_slider.send_keys(Keys.ARROW_RIGHT)
        WebDriverWait(driver, 10).until(lambda _: indicator.text != indicator_before)
        ...
        appendix_d_links = [
            ...
            if "appendix-d-perturbation-theory-response-functions-and-green-s-functions" in (link.get_attribute("href") or "")
        ]
        if not appendix_d_links:
            fail("Appendix C page does not expose navigation to Appendix D")

        assert_no_page_overflow(driver, "Appendix C desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-c-desktop.png"))
        report["desktop"] = {
            "title": title,
            ...
            "keyboard_controls": ["avoided_crossing_gap", "nuclear_velocity"],
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Appendix C narrow-screen bilingual layout is not one column")
        assert_no_page_overflow(driver, "Appendix C narrow screen")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-c-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "appendix-c-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        ...
        static_svgs = driver.find_elements(By.CSS_SELECTOR, ".chapter-visual svg")
        if len(contracts) != 3 or not all(item.is_displayed() for item in contracts):
            fail("No-JavaScript Appendix C page lost a visualization contract")
        if len(static_svgs) != 3 or not all(item.is_displayed() for item in static_svgs):
            fail("No-JavaScript Appendix C page lost a static SVG")
        fallback_rows = driver.find_elements(By.CSS_SELECTOR, ".avoided-crossing__fallback tbody tr")
        if len(fallback_rows) != 3:
            fail(f"Expected three avoided-crossing fallback rows, received {len(fallback_rows)}")
        if len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr")) != 3:
            fail("No-JavaScript Appendix C page lost the source map")
        if len(driver.find_elements(By.CSS_SELECTOR, ".katex")) < 110:
            fail("No-JavaScript Appendix C page lost rendered mathematics")
        assert_no_page_overflow(driver, "Appendix C no-JavaScript desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "appendix-c-no-javascript.png"))
        ...
        save_failure_screenshot(driver, "appendix-c-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
def main() -> int:
    ...
    report_path = ARTIFACT_DIR / "part07-app-c-report.json"
...
if __name__ == "__main__":
    try:
        ...
        (ARTIFACT_DIR / "part07-app-c-failure.json").write_text(
            json.dumps(failure, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        print(f"Part VII Appendix C Pages smoke test failed: {exc}", file=sys.stderr)
        raise
