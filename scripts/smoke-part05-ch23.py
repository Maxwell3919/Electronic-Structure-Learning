#!/usr/bin/env python3
"""Live GitHub Pages smoke test for Martin Part V Chapter 23."""
...
ARTIFACT_DIR = Path(os.environ.get("SMOKE_ARTIFACT_DIR", "artifacts/pages-smoke")) / "chapter-23"
...
CHAPTER_URL = urljoin(BASE_URL, "part-05-properties-of-matter/chapter-23-wannier-functions/")
    ...
    for _ in range(attempts):
        try:
            request = urllib.request.Request(
                url,
                headers={"User-Agent": "Electronic-Structure-Learning-Ch23-Smoke/1"},
            )
...
def part_index_smoke(report: dict) -> None:
    ...
    try:
        ...
        links = driver.find_elements(By.CSS_SELECTOR, 'a[href*="chapter-23-wannier-functions"]')
        if not links:
            fail("Part V index lacks a Chapter 23 link")
        href = links[0].get_attribute("href") or ""
        if "/Electronic-Structure-Learning/" not in href:
            fail(f"Chapter 23 link escaped the Pages base path: {href}")
        ...
        report["part_index"] = {"dependency_stages": stages, "chapter_23_link": href}
    except Exception:
...
def desktop_and_interaction_smoke(report: dict) -> None:
    ...
    try:
        load_with_retry(driver, CHAPTER_URL, "Wannier")
        WebDriverWait(driver, 20).until(lambda active: len(active.find_elements(By.CSS_SELECTOR, ".katex")) > 0)
        if "Chapter 23" not in driver.title:
            fail(f"Unexpected Chapter 23 title: {driver.title}")
        if grid_column_count(driver, ".bilingual-section__grid") != 2:
            fail("Chapter 23 desktop bilingual layout is not two columns")

        katex_count = len(driver.find_elements(By.CSS_SELECTOR, ".katex"))
        if katex_count < 120:
            fail(f"Expected at least 120 rendered KaTeX nodes, found {katex_count}")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows < 8:
            fail(f"Chapter 23 source map must contain at least eight rows, found {source_rows}")
        contents_links = driver.find_elements(By.CSS_SELECTOR, ".chapter23-contents a")
        if len(contents_links) != 13:
            fail(f"Chapter 23 contents must contain thirteen links, found {len(contents_links)}")
        figures = driver.find_elements(By.CSS_SELECTOR, ".chapter23-root .chapter-visual")
        contracts = driver.find_elements(By.CSS_SELECTOR, ".chapter23-root .chapter-visual__contract")
        if len(figures) != 5 or len(contracts) != 5:
            fail(f"Chapter 23 requires five figures/contracts, found {len(figures)}/{len(contracts)}")

        move_control_and_expect_change(
            driver,
            "#ch23-gauge-localization [data-shift]",
            "#ch23-gauge-localization [data-center]",
        )
        move_control_and_expect_change(
            driver,
            "#ch23-composite-gauge [data-theta]",
            "#ch23-composite-gauge [data-x1]",
        )
        move_control_and_expect_change(
            driver,
            "#ch23-overlap-branch [data-gauge]",
            "#ch23-overlap-branch [data-unwrapped]",
        )
        move_control_and_expect_change(
            driver,
        ...
        move_control_and_expect_change(
            driver,
            "#ch23-interpolation-locality [data-cutoff]",
            "#ch23-interpolation-locality [data-max-error]",
        )

        assert_no_page_overflow(driver, "Chapter 23 desktop")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-23-desktop.png"))
        report["desktop"] = {
            "title": driver.title,
            ...
            "keyboard_controls": [
                "integer_gauge_shift",
                "composite_rotation",
                "overlap_gauge_winding",
                "outer_window_minimum",
                "hopping_cutoff",
            ],
        }
        ...
        if grid_column_count(driver, ".bilingual-section__grid") != 1:
            fail("Chapter 23 narrow bilingual layout is not a single column")
        assert_no_page_overflow(driver, "Chapter 23 narrow")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-23-narrow.png"))
        report["narrow"] = {"viewport": [390, 844], "bilingual_columns": 1}
    except Exception:
        save_failure_screenshot(driver, "chapter-23-desktop-failure.png")
        raise
    finally:
        driver.quit()
...
def no_javascript_smoke(report: dict) -> None:
    ...
    try:
        ...
        if len(contracts) != 5:
            fail(f"Chapter 23 must retain five visualization contracts, found {len(contracts)}")
        if len(figures) != 5:
            fail(f"Chapter 23 must retain five teaching figures, found {len(figures)}")
        if len(fallbacks) < 5:
            fail(f"Chapter 23 must expose five no-JavaScript explanations, found {len(fallbacks)}")
        if not all(item.is_displayed() for item in contracts + figures):
            fail("A Chapter 23 no-JavaScript figure or contract is hidden")
        source_rows = len(driver.find_elements(By.CSS_SELECTOR, ".chapter-source-map tbody tr"))
        if source_rows < 8:
            fail("No-JavaScript Chapter 23 source map lost one or more rows")
        assert_no_page_overflow(driver, "Chapter 23 no-JavaScript")
        driver.save_screenshot(str(ARTIFACT_DIR / "chapter-23-no-javascript.png"))
        ...
        save_failure_screenshot(driver, "chapter-23-no-javascript-failure.png")
        raise
    finally:
        driver.quit()
...
if __name__ == "__main__":
    try:
        ...
        print(f"Chapter 23 Pages smoke test failed: {exc}", file=sys.stderr)
        raise
