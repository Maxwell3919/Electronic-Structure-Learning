...
const isAcceptedDeclaration = (status) => canonicalStatuses.has(status) || status.endsWith('-complete');
...
assert.match(sourceNote, /endsWith\('-complete'\)/, 'SourceNote must recognize legacy *-complete declarations');
assert.match(sourceNote, /canonicalStatus = 'validated'/, 'Legacy complete declarations need validated canonical semantics');
...
assert.match(sourceNote, /data-status=\{canonicalStatus\}/);
assert.match(sourceNote, /data-declared-status=\{rawStatus\}/);
...
for (const filePath of walk(join(repositoryRoot, 'src'))) {
  ...
  for (const match of matches) {
    ...
    assert.ok(isAcceptedDeclaration(match[1]), `${path} uses unsupported SourceNote status ${match[1]}`);
  }
}
...
assert.ok(!partIndex.includes('chapter-complete'), 'Part I synthesis must not use a legacy complete status');

console.log('Part I closure validation passed: canonical Part I statuses, legacy-status semantics, source locators, five chapter links, bilingual accessibility labels, and the Part II handoff are consistent.');
