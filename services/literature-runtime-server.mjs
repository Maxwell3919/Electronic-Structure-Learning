...
const serveHealth = (request, response) => {
  ...
  return jsonResponse(request, response, 200, {
    ...
    readable_pdf_papers: papers.size,
...
const server = createServer((request, response) => {
  ...
  if (url.pathname === '/papers/' && ['GET', 'HEAD'].includes(request.method ?? '')) return jsonResponse(request, response, 200, {
    service: 'electronic-structure-atlas-literature', published_papers: papers.size, readable_pdf_papers: papers.size, pdf_identity: 'preindexed-sha256-integrity-gate', curated_annotation_store: 'github-records-read-only', personal_annotation_store: 'browser-indexeddb', annotation_api: '/papers/api/annotations/{document_sha256}', health: '/papers/health',
  });
...
server.listen(port, host, () => console.log(`Atlas literature runtime listening on http://${host}:${port} for ${papers.size} integrity-validated canonical PDFs; curated annotations are read-only Records data`));
